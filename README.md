...
| 항목 | 분류 | 제목 | 파일 | 유튜브 | 설명 |
|---|---|---|---|---|---|
| KLS 기초 한국어 선이수제 | 국어 | 1학기 수업계획 | (드라이브 파일 링크) | | |
| KLS 기초 한국어 선이수제 | 국어 | 수업 영상 | | https://youtu.be/… | |
...
- (고급) 같은 분류 안에서 한 번 더 나눠야 하면 `소분류` 열을 추가 → 자료 목록 위에 [전체][1학년]… 칩 버튼이 생김
...
없는 항목, 없는 파일, PDF가 아닌 파일, 25MB 초과 등이 있으면 행 번호와 함께 알려주고 멈춥니다. 파일·유튜브가 둘 다 빈 줄(입력 중)은 건너뜁니다.
