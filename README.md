...
- **단계·항목**: 드롭다운. 단계를 고르면 그 단계 항목만 보이고, 항목을 먼저 고르면 단계가 자동으로 채워짐
...
시트 설정은 `tools/sheet/Code.gs`를 시트의 Apps Script(Code.gs)에 붙여 넣은 뒤
메뉴 **[사이트 반영 → 시트 구성 정리]**를 한 번 누르면 열·목록·안내 탭·드롭다운·1행 보호가 한꺼번에 설정됩니다.
그 뒤로는 **[사이트 반영 → 지금 사이트에 반영하기]**로 배포합니다 (Cloudflare 배포 훅).
...
로드맵 항목 이름을 바꾸면 `index.html`과 `tools/sheet/Code.gs`의 `ROADMAP`을 같이 고치고 [시트 구성 정리]를 다시 누르세요.
